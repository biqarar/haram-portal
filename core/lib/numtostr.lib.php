...
class numtostr_lib {
	...
	public function __construct(){
		$this->count_str = array(
							...
							// 10 => "add name for convert :))",
							// 11 => "add name for convert :))",
							// 12 => "add name for convert :))",
							// 13 => "add name for convert :))",
							// 14 => "add name for convert :))",
							// 15 => "add name for convert :))",
							// 16 => "add name for convert :))",
							// 17 => "add name for convert :))",
							// 18 => "add name for convert :))",
							// 19 => "add name for convert :))",
							// 20 => "add name for convert :))",
							// 21 => "add name for convert :))",
							// 22 => "add name for convert :))",
							// .. => "....   :))",
							// .. => "....   :))",
							// .. => "....   :))",
							// .. => "....   :))",
	...
	public function convert($value = false) {
		if(strlen($value) > $this->max_num) {
			debug_lib::fatal("<a style='color:red'>عدد وارد شده نباید بیشتر از {$this->max_num} رقم باشد</a>");
		}
		...
		if(preg_match("/^(\-\d+)|(\d+\-)$/", $value)){
			...
			if($value == 0) $this->manfi = false;
			
		}

		if(!preg_match("/^\d+$/", $value)){
			debug_lib::fatal("<a style='color:red'>لفطا فقط از کاراکتر های عددی استفاده کنید</a>");
		}
