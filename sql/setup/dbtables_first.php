<?php
$connect = mysqli_connect("localhost", "root", "root", "quran_hadith");
$qTables = $connect->query("SHOW TABLES FROM quran_hadith");
function _type($type, $def){
	...
	while ($row = $qTables->fetch_object()) {
		...
		$TABLENAME	= $row->Tables_in_quran_hadith;
		...
		$content .= "}\n";
		$content .= "?>";
		file_put_contents("sql_temp/$TABLENAME.sql.php", $content);
	}
	$connect->close();
